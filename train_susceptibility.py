"""
SentryMesh - Landslide Susceptibility Model (GPU PyTorch version)

Trains on data/susceptibility.parquet from build_susceptibility.py.
...
CFG = {
    ...
    "model_path": "checkpoints/landslide_susceptibility_model.pt",
    "metrics_path": "checkpoints/susceptibility_metrics.json",
}
...
def main():
    ...
    df = pd.read_parquet("data/susceptibility.parquet")
    ...
    print("\nLandslide Susceptibility - Test Results")
    for k, v in m.items():
        print(f"  {k:10s}: {v:.4f}" if isinstance(v, float) else f"  {k:10s}: {v}")
    print("\n", classification_report(y_te, pred, target_names=["No event", "Landslide"], zero_division=0))
